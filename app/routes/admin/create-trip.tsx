...
export const loader = async () => {
    ...
    return data.map((country: any) => ({ name: country.flag + country.name.common, coordinates: country.latlng, value: country.name.common, openStreetMap: country.maps?.openStreetMap }))
}
const CreateTrip = ({ loaderData }: Route.ComponentProps) => {
    ...
    const mapData = [
        { country: formData.country, color: '#EA382E', coordinates: countries.find((c: Country) => c.name === formData.country)?.coordinates || [] }
    ]

    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault()
        setLoading(true)
        if (!formData.country || !formData.travelStyle || !formData.groupType || !formData.interest || !formData.budget) {
            ...
            <section className="mt-2.5 wrapper-md">
                <form className="trip-form" onSubmit={handleSubmit}>
                    ...
                    <div>
                        ...
                        <input id="duration" type="number" name="duration" placeholder="Enter a number of days(5, 12...)" className="form-input placeholder:text-gray-100" onChange={(e) => handleChange('duration', Number(e.target.value))} />
                    </div>
                    {
                        selectItems.map((key) => (
                            <div key={key}>
